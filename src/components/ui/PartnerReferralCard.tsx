...
export default function PartnerReferralCard() {
  ...
  const loadUserAndProspects = async () => {
    try {
      ...
      // Buscar prospects indicados pelo parceiro
  ...
  const getLandingPageUrl = () => {
    if (!user?.partner_slug) return ''
    const baseUrl = window.location.origin + window.location.pathname
    return `${baseUrl}#parceiro/${user.partner_slug}`
  }
  ...
  const shareUrl = async () => {
    const url = getLandingPageUrl()
    const text = `Seja um parceiro! Acesse: ${url}`
  ...
  if (!user?.partner_slug) {
    return null
  }

  return (
    <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-lg shadow-lg p-6 border border-indigo-100">
      {/* Header */}
        ...
        <div>
          ...
          {prospects.length > 3 && (
            <p className="mt-2 text-center text-sm text-gray-500">
              e mais {prospects.length - 3} indicações...
            </p>
          )}
        </div>
      )}
      ...
      {prospects.length === 0 && (
        <div className="bg-white rounded-lg p-6 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Compartilhe sua landing page e comece a recrutar novos parceiros!
          </p>
        </div>
      )}
    </div>
